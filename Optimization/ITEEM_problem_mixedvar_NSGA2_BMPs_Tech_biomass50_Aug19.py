...
class ITEEM_problem(Problem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        solution = ITEEM(landuse_matrix = landuse_matrix, tech_wwt=tech_wwt_list[x[-4]],
                        limit_N = 10.0, tech_GP1=tech_GP_list[x[-3]], 
                        tech_GP2=tech_GP_list[x[-2]], tech_GP3=tech_GP_list[x[-1]])
...
'''save data'''
import scipy.io
scipy.io.savemat('C:\ITEEM\Optimization\solutions\opt_F_NSGA2_BMPs_Tech_biomass50_Sept13_2021.mat', mdict={'out': opt_F}, oned_as='row')
scipy.io.savemat('C:\ITEEM\Optimization\solutions\opt_X_NSGA2_BMPs_Tech_biomass50_Sept13_2021.mat', mdict={'out': opt_X}, oned_as='row')

'''post analysis'''
opt_X = scipy.io.loadmat(r'C:\ITEEM\Optimization\solutions\opt_X_NSGA2_BMPs_Tech_biomass50_Sept13_2021.mat')['out']
...
import pandas as pd
opt_X = scipy.io.loadmat(r'C:\ITEEM\Optimization\solutions\opt_X_NSGA2_BMPs_Tech_biomass50_Sept13_2021.mat')['out']
landuse_matrix_BMP_tech = opt_landuse('NSGA2_BMPs_Tech_biomass50_Sept13_2021')
# landuse_matrix_BMP_tech = landuse_matrix
...
for i in range(100):
    print(i)
    wwt_tech = opt_X[:,70].astype('int')
    gp_tech1 = opt_X[:,71].astype('int')
    gp_tech2 = opt_X[:,72].astype('int')
    gp_tech3 = opt_X[:,73].astype('int')
    ...
    run_ITEEM_opt = output.run_ITEEM_opt(sg_price=0.050)
    df_spider_output2['run'+str(1+i)] = run_ITEEM_opt[-1]
    df_obj2['run'+str(1+i)] = [run_ITEEM_opt[0], run_ITEEM_opt[1], run_ITEEM_opt[2], run_ITEEM_opt[3], run_ITEEM_opt[4]]
    
mean = df_spider_output2.mean(axis=1)

# df = np.array(df_spider_output2.T)
# df_bimoass_tech = scipy.io.loadmat(r'C:\ITEEM\Optimization\solutions\opt_F_NSGA2_BMPs_Tech_biomass50_Aug19_2021_DWT.mat')['out']

df = np.array(df_obj2.T)
scipy.io.savemat('C:\ITEEM\Optimization\solutions\opt_F_NSGA2_BMPs_Tech_biomass50_Sept13_2021.mat', mdict={'out':df}, oned_as='row')


'''Start section: calculate fold of WTP'''
baseline_benefit = 529783859.9  # $/yr
fold50 = []
for i in range(df_spider_output2.shape[1]):
    a = (baseline_benefit - df_spider_output2.iloc[-4,i])/df_spider_output2.iloc[-5,i]
    fold50.append(a)
    # print(fold)
...
# scipy.io.savemat('C:\ITEEM\Optimization\solutions\hv_NSGA2_BMPs_Tech_biomass50_Sept13_2021.mat', mdict={'out':hv_n_evals}, oned_as='row')
