...
# from Submodel_WWT.get_distribution_para import *
from Submodel_WWT.SDD_analysis.influent_SDD import influent_SDD, influent_SDD_multiyear2, influent_SDD_ave
from Submodel_WWT.SDD_analysis.wwt_cost_energy_functions import blower_energy, chemical_cost, heating_energy, sludge_cost, fix_energy_cost
# from Submodel_SWAT.SWAT_functions import loading_outlet_USRW
# from Submodel_WWT.SDD_analysis.results_plot_SDD import *

# set global variable
df_AS = pd.read_csv('C:\ITEEM\Submodel_WWT\simulation_data_csv\simulation_10000runs_AS_Dec.csv', skiprows=range(1,2))
df_ASCP = pd.read_csv('C:\ITEEM\Submodel_WWT\simulation_data_csv\simulation_10000runs_ASCP_Dec.csv', skiprows=range(1,2))
df_EBPR_basic = pd.read_csv('C:\ITEEM\Submodel_WWT\simulation_data_csv\simulation_10000runs_EBPR_basic_Oct.csv', skiprows=range(1,2))
df_EBPR_acetate = pd.read_csv('C:\ITEEM\Submodel_WWT\simulation_data_csv\simulation_10000runs_EBPR_acetate_Oct.csv', skiprows=range(1,2))
df_EBPR_StR = pd.read_csv('C:\ITEEM\Submodel_WWT\simulation_data_csv\simulation_10000runs_EBPR_StR_Oct.csv', skiprows=range(1,2))

model_AS = load_model(r'C:\ITEEM\Submodel_WWT\model_AS.h5')
model_ASCP = load_model(r'C:\ITEEM\Submodel_WWT\model_ASCP.h5')
model_EBPR_basic = load_model(r'C:\ITEEM\Submodel_WWT\model_EBPR_basic.h5')
model_EBPR_acetate = load_model(r'C:\ITEEM\Submodel_WWT\model_EBPR_acetate_Dec_exlowN.h5')
model_EBPR_StR = load_model(r'C:\ITEEM\Submodel_WWT\model_EBPR_StR_Dec_exlowN.h5')
...
# tech_ebpr_sim = tech_ebpr.get_cost_energy_nutrient(1000, landuse_matrix_baseline, r=0.07, n_wwt=40,
#                                                 nutrient_index=1.0, flow_index=1.0, chem_index=1.0, 
#                                                 utility_index=1.0, rP_index=1.0)
# tech_ebpr_sim[4].mean()/(10**6)
# tech_ebpr_sim = tech_ebpr.get_cost_energy_nutrient(1000, landuse_matrix_baseline, r=0.07, n_wwt=40,
#                                                 nutrient_index=1.0, flow_index=1.0, chem_index=1.0, 
#                                                 utility_index=1.0, rP_index=1.0)
# energy_aer_kwh = tech_ebpr_sim[5][0]/(10**6)*0.277778 # kwh/yr
# energy_aer_kwh.min();energy_aer_kwh.max() 

# tech_as = WWT_SDD(tech='AS', multiyear=True, start_yr = 2003, end_yr=2018)
# output_scaled, outputs, influent = tech_as.run_model(1000, nutrient_index=1.0, flow_index=1.0)
# nitrate_eff =  (outputs[:,:,0]*influent[:,:,0]/1000).mean()*365  # kg/yr, singular
# tp_eff = (outputs[:,:,2]*influent[:,:,0]/1000).mean()*365  # kg/yr, singular
# tp_inf = (influent[:,:,1]*influent[:,:,0]/1000).mean()*365 # kg/yr
# nutrient_red = [nitrate_eff, tp_eff, tp_inf]

# tech = WWT_SDD(tech='EBPR_StR', multiyear=True, start_yr = 2003, end_yr=2018)
# landuse_matrix_baseline = np.zeros((45,62))
# tech_sim = tech.get_cost_energy_nutrient(1000, landuse_matrix_baseline, r=0.07, n_wwt=40,
#                                                 nutrient_index=1.0, flow_index=1.0, chem_index=1.0, 
#                                                 utility_index=1.0, rP_index=1.0)
# tech_as_sim[-2][:,:,33].sum(axis=1).mean()
# wwt_baseline = tech_as.get_cost_energy_nutrient(1000, landuse_matrix_baseline, r=0.07, n_wwt=40)
...
# loading_day_nitrate = nitrate_load.mean(axis=1)/1000
# loading_day_nitrate_matrix = loading_day_nitrate.reshape(16,12)

# tp_load = output_raw[:,:,2]*influent_tot[:,:,0]
# loading_day_tp = tp_load.mean(axis=1)/1000  # loading: kg/d
# loading_day_tp_matrix = loading_day_tp.reshape(16,12)

# import scipy.io
# scipy.io.savemat('ASCP_nitrate_matrix.mat', mdict={'out': loading_day_nitrate_matrix}, oned_as='row')
# scipy.io.savemat('ASCP_tp_matrix.mat', mdict={'out': loading_day_tp_matrix}, oned_as='row')
# matdata = scipy.io.loadmat('ASCP_nitrate_matrix.mat')
# score_nitrate_load = matdata['out']

# landuse_matrix = np.zeros((45,56))
# landuse_matrix[:,0]=1
# start = time.time()
# b = a.get_cost_energy(sample_size=10, r= 0.07, n=40)
# nitrate, tp2 = a.run_model_USRW(sample_size=100, landuse_matrix=landuse_matrix)
# tp2[:,:,33].sum(axis=1).mean()

# c = a.get_cost_energy(1000, landuse_matrix, 0.03, 40)
# outlet_tp = c[-1]
# outlet_nitrate = c[-2]
# a.run_model_USRW(1000, landuse_matrix)
# end = time.time()
# print('Simulation time is: ', end - start)


'''calculate cost with uncertainty'''
# tech_ebpr_str = WWT_SDD(tech='EBPR_StR', multiyear=True, start_yr = 2006, end_yr=2015)
# cost_energy_ebpr_str = tech_ebpr_str.get_cost_energy_ave(1000, 0.07, 40)
# cost_energy_ebpr_str[2][3] # chemical mean
# np.percentile(cost_energy_ebpr_str[3][1],95) # chemical upper bound
# np.percentile(cost_energy_ebpr_str[3][1],5)  # chemical lower bound
# cost_energy_ebpr_str[0]  # total cost
# np.percentile(cost_energy_ebpr_str[1],95)    # total cost upper
# np.percentile(cost_energy_ebpr_str[1],5)     # total cost lower
# cost_energy_ebpr_str[-1][1].mean() 

# influent_tot = influent_SDD_multiyear2(100, start_yr = 2003, end_yr = 2018)
# influent_tot2 = influent_tot.mean(axis=1)[:,0]
# nitrate_load = output_raw[:,:,0]*influent_tot[:,:,0]
# instance = WWT_SDD(tech='ASCP', multiyear=True, start_yr = 2003, end_yr=2018)
# output_scaled, output_raw, influent_tot = instance.run_model(sample_size=100)
# start = time.time()
...
#     tech = WWT_SDD(tech=tech_list[i], multiyear=True, start_yr = 2003, end_yr=2018)
    # cost_energy = tech.get_cost_energy(sample_size=1000, r= 0.07, n=40)
    # cost_tech = cost_energy[1]
    # energy_tech = cost_energy[2]
    # nutrient_tech = cost_energy[-1]
    # struvite_tech = tech.get_rP_struvite()
    # cost.append(cost_tech)
    # energy.append(energy_tech)
    # nutrient.append(nutrient_tech)
    # struvite.append(struvite_tech)
...
'''write outputs to the excel file'''
# writer = pd.ExcelWriter('output_'+ self.tech + '.xlsx', engine='xlsxwriter')
# for i in range(12):
#     df = pd.DataFrame(output_raw[i,:,:])
#     df.to_excel(writer, sheet_name='Month_' + str(i+1))
# writer.save()
# a numpy array: output_raw by order: Nitrate, TSS, TP, COD, TN
    
# class WWT_TEA_SDD(WWT_ANN):
#     def __init__(self,tech):
#         WWT_ANN.__init__(self,tech)
# A = WWT_TEA_SDD(tech='ASCP')
# issubclass(WWT_TEA, WWT_ANN)