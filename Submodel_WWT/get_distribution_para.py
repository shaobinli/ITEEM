...
def get_monthly_ave(name):
    ...
    if name == 'effluent':
        df = pd.read_excel(r'C:\ITEEM\Submodel_WWT\SSD_effluent.xlsx', parse_dates=['Date'], index_col='Date')
        df = df.dropna()
        df = df[(df.iloc[:,0]<115)]
        ...
        df['year'] = df.index.year
         ...
         df = pd.read_excel(r'C:\ITEEM\Submodel_WWT\SSD_effluent.xlsx', parse_dates=['Date'], index_col='Date', sheet_name=1)
         df = df.dropna()
         df_min, df_max = min(df.iloc[:,0]), max(df.iloc[:,0])  
         df['month'] = df.index.month
         df['year'] = df.index.year
...
def get_lognormal_para(name):
    name = 'Inflow'
    df = pd.read_excel(r'C:\ITEEM\Submodel_WWT\SDD_N_P_2012-2019.xlsx', 
