...
from Submodel_SWAT.SWAT_functions import basic_landuse, pbias, nse
from Submodel_SWAT.results_validation_originalRM import loading_outlet_originalRM

# df = pd.read_excel(r'C:\ITEEM\Submodel_SWAT\results_validation\100Randomizations\100RandomizeHRUstorePercentage_5BMPsDec82020.xlsx')
xls = pd.ExcelFile(r'C:\ITEEM\Submodel_SWAT\results_validation\100Randomizations\100RandomizeHRUstorePercentage_5BMPsDec82020.xlsx')
...
def landuse_mat(*args):
    ...
    # df = pd.read_csv(r'C:\ITEEM\Submodel_SWAT\results_validation\response_matrix_csv\yield_nitrate.csv')
    df = pd.read_csv(r'C:\ITEEM\Submodel_SWAT\response_matrix_csv\yield_nitrate.csv')
...
def get_area_prcnt(sheet_name):
    ...
    '''
    # df = pd.read_excel(r'C:\ITEEM\Submodel_SWAT\results_validation\StorePercentageHRU_First5BMPs.xlsx', 
...
def sediment_instream(sw, scenario_name):
    ...
    pd_coef_poly = pd.read_excel(r'C:\ITEEM\Submodel_SWAT\results_validation\sediment_streamflow_regression_coefs.xlsx', sheet_name='poly', usecols='B:D')
...
def swat_vs_iteem(name, sw, ag_scenario, plot=True):
    ...
    if name == 'nitrate':
        # df = pd.read_excel(r'C:\ITEEM\Submodel_SWAT\Response_matrix_BMPs.xlsx',sheet_name=0)
        df = pd.read_csv(r'C:\ITEEM\Submodel_SWAT\results_validation\100Randomizations\loading_nitrate.csv')
    elif name == 'phosphorus':
        df = pd.read_csv(r'C:\ITEEM\Submodel_SWAT\results_validation\100Randomizations\loading_phosphorus.csv')
    elif name == 'sediment':
        df = pd.read_csv(r'C:\ITEEM\Submodel_SWAT\results_validation\100Randomizations\loading_sediment.csv')
    elif name == 'streamflow':
        df = pd.read_csv(r'C:\ITEEM\Submodel_SWAT\results_validation\100Randomizations\loading_streamflow.csv')
        df = df*30*60*60*24
    subwatershed = df.iloc[:,0].unique()
    year = df.iloc[:,1].unique()
    month = df.iloc[:,2].unique()
...
#            df = df.reset_index(inplace=False, drop= True)
    ...
    if plot == True:
        ...
        plt.tight_layout()
        plt.savefig(r'C:\ITEEM\ITEEM_figures\RM_Jan_2021\randomized\ITEEM_method'+name+'_' + ag_scenario +'_sw'+str(sw+1) +'_coefficientonly.tif', dpi=300)
        plt.show()
